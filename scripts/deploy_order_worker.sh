...
SERVICE_DIR="$ROOT_DIR/services/order-worker"

AWS_REGION="${AWS_REGION:-us-east-1}"
PROJECT="${PROJECT:-flash-sale}"
ECR_REPO_NAME="${ECR_REPO_NAME:-${PROJECT}-order-worker}"
ECS_CLUSTER="${ECS_CLUSTER:-${PROJECT}-cluster}"
ECS_SERVICE="${ECS_SERVICE:-${PROJECT}-order-worker}"
...
echo "==> Building order-worker image (${IMAGE_TAG})"
