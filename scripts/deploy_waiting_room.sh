...
SERVICE_DIR="$ROOT_DIR/services/waiting-room"

AWS_REGION="${AWS_REGION:-us-east-1}"
PROJECT="${PROJECT:-flash-sale}"
ECR_REPO_NAME="${ECR_REPO_NAME:-${PROJECT}-waiting-room}"
ECS_CLUSTER="${ECS_CLUSTER:-${PROJECT}-cluster}"
ECS_SERVICE="${ECS_SERVICE:-${PROJECT}-waiting-room}"
...
echo "==> Building waiting-room image (${IMAGE_TAG})"
